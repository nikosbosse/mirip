#' @title Load Counts Data From Single Files
#'
#' @description
#' Load read counts from a list of text files. Each text file must contain the
#' reads for one sample and all samples must have the same miRNA in exactly the
#' same order. Files are expected to be sorted in the following way:
#' input1, ..., input_n, IP1, ..., IP_n.
#' @param path Path to the folder where the count files are located
#' @param patter file extension that marks the count files to be parsed.
#' @return data frame with all counts
#' @author Nikos Bosse \email{nikosbosse@gmail.com}
#' @export


load_data_from_files <- function(path = "../data/",
                                 pattern = ".txt"){

  all.files <- list.files(path = path, pattern = pattern)
  mylist <- lapply(all.files,
                   function(x){read.table(paste(path, x, sep = ""),
                                          row.names = 1)})
  counts <- do.call("cbind",mylist)

  cols_input <- numeric(ncol(counts) / 2)
  cols_ip <- numeric(ncol(counts) / 2)
  for (i in 1:length(cols_input)) {
    cols_input[i] <- cat("input", i)
    cols_ip[i] <- cat("input", i)
  }

  colnames(counts) <- c(cols_input, cols_ip)
...
#' @title GMPR Normalization
#'
#' @description
#' Implementation of the GMPR Normalization method (CITATION)
#' @param counts matrix or data.frame with read counts of dimension (Nxn),
#' where N is the number of miRNA and n is the number of samples.
#' @return vector of length n with weights given to the respective samples.
#' @author Nikos Bosse \email{nikosbosse@gmail.com}
#' @export
...
#'
#' @description
#' Re-Implementation of the DESeq2 Normalization method (CITATION)
#' @param counts matrix or data.frame with read counts of dimension (Nxn),
#' where N is the number of miRNA and n is the number of samples.
#' @return vector of length n with weights given to the respective samples.
#' @author Nikos Bosse \email{nikosbosse@gmail.com}
...
deseq2_normalization <- function(counts) {
...
#' @title Generate a list that can be processed by STAN
#'
#' @description
#' MISSING
#'
#' @param counts matrix or data.frame with read counts of dimension (Nxn),
#' where N is the number of miRNA and n is the number of samples.
#' @return list with the following components: MISSING
#' @author Nikos Bosse \email{nikosbosse@gmail.com}
#' @export
...
generatestanobject <- function(counts, s_to_one = F, g_to_one = T){
  ...
  if (!is.matrix(counts)) {
    stop("counts must be a matrix (preferred) or a data.frame")
  }
